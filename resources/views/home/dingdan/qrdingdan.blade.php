...
<div class="ucenter">
    <div class="leftnnav">
	    <dl>
	        ...
	        <dd><a class="cur" title="评价晒单" href="/home/dingdan/qr">评价晒单</a></dd>
	    </dl>
	    <dl>
	        <dt>个人中心</dt>
    ...
    <div style="min-height:599px;overflow: visible;" class="rMain">
	        ...
	        <!--广告位-->
	            ...
	            <a href="/home/dingdan"><span style="color:#666" class="all-order">全部订单</span></a>|<a href="/home/dingdan/qr"><span style="color:#d70057" class="waite-order">待评价晒单</span></a>
	        </p>
	        <!--无子订单模板start-->
	        @foreach($res2 as $v)
	    	...
	    	<tbody>
				<tr class="titl pinkbgbor">
		        	<td width="150"><span class="ordergoing-status">已确认收货</span></td>
		            <td width="548">
		            	<p class="pay-orderinfo">订单编码：
		            	<a target="_blank" class="ordercode">{{$v['order_num']}}</a>
		            	<span class="sep">|</span>{{$v['time']}}<span tclass="sep"> | </span>{{$v['to_name']}}&nbsp;（收货人）
		            	</p>
		            </td>
	        ...
	        <!--循环子订单中的商品列表，展示商品清单-->
			  @foreach(App\Http\Controllers\home\DingdanController::getOrderxq($v['id']) as $val)
	            ...
	            <td class="order-operate">
					...
					<!--去支付链接-->
					<div style="width:100px;left:85px;top:15px;" class="order-operate">
						@if($val['sus'] == 0)
						<form action="/pinglun/add" method="post">
							{{csrf_field()}}
							<input type="hidden" name="gid" value="{{$val['gid']}}">							
							<input type="hidden" name="id" value="{{$val['id']}}">							
							<input type="hidden" name="title" value="{{$val['title']}}">							
							<input type="hidden" name="oid" value="{{$val['oid']}}">
							<input type="submit" style="background:red;height:30px;border:0px;border-radius:5px;color:#fff" value="去评论">
						</form>
						@else
						
							
							<input type="button" style="background:red;height:30px;border:0px;border-radius:5px;color:#fff" value="已评论">
